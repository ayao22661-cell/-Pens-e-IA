    ...
    if (files && files.length > 0) {
        ...
        files.forEach(file => {
            if (file.content && file.content.type === "text") {
                // ── Troncature intelligente des fichiers texte volumineux ──────
                // Un fichier texte brut injecté tel quel dans le prompt peut
                // dépasser la fenêtre contextuelle du modèle (1MB ≈ 250K tokens).
                // Stratégie : on garde DÉBUT + FIN du fichier (les deux zones les
                // plus importantes pour comprendre la structure d'un fichier de code)
                // et on insère un résumé du milieu compressé.
                const MAX_CHARS = 60000; // ~15K tokens — confortable pour tous les modèles
                const content   = file.content.data;

                if (content.length <= MAX_CHARS) {
                    // Fichier petit → injection complète
                    userPrompt += "DOCUMENT : " + file.name + "\nCONTENU :\n" + content + "\n---\n\n";
                } else {
                    // Fichier volumineux → head + tail avec note de compression
                    const HEAD     = 25000; // 25K chars du début (imports, classes, structure)
                    const TAIL     = 25000; // 25K chars de la fin (logique principale)
                    const head     = content.slice(0, HEAD);
                    const tail     = content.slice(-TAIL);
                    const skipped  = content.length - HEAD - TAIL;
                    const pct      = Math.round((skipped / content.length) * 100);
                    const lines    = content.split('\n').length;
                    const linesHead = head.split('\n').length;
                    const linesTail = tail.split('\n').length;

                    userPrompt += `DOCUMENT : ${file.name} (${(content.length / 1024).toFixed(0)}KB — ${lines} lignes)\n`;
                    userPrompt += `[⚡ Fichier volumineux : début + fin affichés. ${pct}% du milieu compressé.]\n`;
                    userPrompt += `\n--- DÉBUT DU FICHIER (lignes 1–${linesHead}) ---\n`;
                    userPrompt += head;
                    userPrompt += `\n\n--- [${skipped.toLocaleString()} caractères intermédiaires omis] ---\n\n`;
                    userPrompt += `--- FIN DU FICHIER (dernières ~${linesTail} lignes) ---\n`;
                    userPrompt += tail;
                    userPrompt += "\n---\n\n";
                    userPrompt += `[NOTE] Si tu as besoin d'une section précise du fichier, demande à l'utilisateur de la copier-coller.\n\n`;
                }
            } else {
...
// ── Estimation de complexité côté client (miroir de chat.js) ──
// Utilisé pour le scoring knowledge — pas besoin d'être précis.
function estimatePromptComplexity(prompt) {
    ...
    const signals = [
        ...
        /système|application|projet/i,
        /compare|analyse|audit/i,
        /algorithme|performance/i,
    ];
    score += signals.filter(p => p.test(prompt)).length;
    return Math.min(score, 5);
}

// Helper : retire les balises <think>...</think> du texte affiché dans la bulle
// (le contenu est déjà géré par le panneau thinking-ui)
function _stripThinkTags(text) {
    return text
        .replace(/<think>[\s\S]*?<\/think>/gi, '')
        .replace(/<think>[\s\S]*/gi, '')  // Bloc ouvert non fermé (stream en cours)
        .replace(/^\n+/, '');
}
        ...
        try {
            ...
            if (searchRes.ok) {
                ...
                if (webSources.length > 0) {
                    ...
                    const FETCH_COUNT = 2;
                    const CHAR_LIMIT  = 3000; // par source — total ~6000 chars injectés
...
if (!response.ok) {
    ...
    // Message contextualisé selon le code HTTP
    if (response.status === 413) {
        // 413 = Payload Too Large — envoyé par chat.js quand token overflow détecté
        // Le message vient du serveur, on l'affiche tel quel (déjà lisible)
    } else if (response.status === 503) {
        // 503 avec un fichier lourd → message plus clair que "saturé"
        const hasLargeFile = files && files.some(f =>
            f.content?.type === 'text' && f.content?.data?.length > 40000
        );
        if (hasLargeFile) {
            errMsg = "Fichier trop volumineux pour être analysé en entier. "
                   + "Copie-colle uniquement la section qui t'intéresse (une classe, une fonction).";
        }
    }

    // ✅ Pas de creditsLeft-- : le serveur n'a pas répondu → crédit non consommé
    removeTyping();
    addMessage("bot", "· " + errMsg, false);
    setStatus("err");
    return;
}
        ...
        // ── Lecture des headers v4 ──────────────────────────────
        const xModel     = response.headers.get('X-Model-Used')  || '';
        const xComplex   = response.headers.get('X-Complexity')  || '';
        const xThink     = response.headers.get('X-ThinkBudget') || '0';
        const hasThinking = parseInt(xThink, 10) > 0;

        // Badge modèle discret sous le label de la bulle
        if (xModel) {
            const modelBadge = document.createElement('span');
            modelBadge.style.cssText = 'font-family:"JetBrains Mono",monospace;font-size:9px;color:var(--text3);letter-spacing:0.06em;margin-left:8px;opacity:0.7;';
            modelBadge.textContent = xModel.replace('+critique', ' ✦');
        ...
        // ── États du stream think ───────────────────────────────
        let _inThinkBlock   = false;
        let _thinkContent   = "";
        let _thinkStartTime = null;
        let _thinkTimerEl   = null;
        let _thinkWrapper   = null;
        let _thinkLiveEl    = null;
        let _thinkTimerInt  = null;
        let _emotionHandled = false;
        ...
        // ── Helpers thinking UI ─────────────────────────────────
        const _showThinkLive = () => localStorage.getItem('pensee_show_thinking') !== 'false';

        function _openThinkPanel() {
            if (_thinkWrapper) return; // Déjà ouvert
            _thinkStartTime = Date.now();

            // Wrapper inséré AVANT la bulle dans le msgDiv
            _thinkWrapper = document.createElement('div');
            _thinkWrapper.className = 'pensee-thinking-wrapper';

            const statusRow = document.createElement('div');
            statusRow.className = 'pensee-thinking-status';
            statusRow.innerHTML = `
                <span class="pensee-thinking-orb"></span>
                <span class="pensee-status-text">Réflexion en cours</span>
                <span class="pensee-thinking-timer">0s</span>
            `;
            _thinkTimerEl = statusRow.querySelector('.pensee-thinking-timer');

            const liveZone = document.createElement('div');
            liveZone.className = 'pensee-thinking-live' + (_showThinkLive() ? ' expanded' : '');
            _thinkLiveEl = document.createElement('div');
            _thinkLiveEl.className = 'pensee-thinking-live-inner';
            liveZone.appendChild(_thinkLiveEl);

            _thinkWrapper.appendChild(statusRow);
            _thinkWrapper.appendChild(liveZone);
            msgDiv.insertBefore(_thinkWrapper, bubble);

            _thinkTimerInt = setInterval(() => {
                if (_thinkTimerEl) {
                    _thinkTimerEl.textContent = Math.floor((Date.now() - _thinkStartTime) / 1000) + 's';
                }
            }, 1000);
        }

        function _closeThinkPanel(finalContent) {
            clearInterval(_thinkTimerInt);
            if (!_thinkWrapper) return;
            const dur = Math.floor((Date.now() - _thinkStartTime) / 1000);
            const durLabel = dur >= 60 ? `${Math.floor(dur/60)}m ${dur%60}s` : `${dur}s`;

            const block = document.createElement('div');
            block.className = 'pensee-think-block';
            block.innerHTML = `
                <button class="pensee-think-toggle" aria-expanded="false">
                    <svg viewBox="0 0 20 20" width="11" height="11" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round">
                        <path d="M10 2a6 6 0 0 1 6 6c0 2.5-1.5 4.7-3.7 5.6L12 16H8l-.3-2.4C5.5 12.7 4 10.5 4 8a6 6 0 0 1 6-6z"/>
                        <line x1="8" y1="18" x2="12" y2="18"/>
                    </svg>
                    Analyse interne
                    <span class="pensee-think-duration">${durLabel}</span>
                    <svg class="pensee-think-chevron" viewBox="0 0 20 20" width="10" height="10" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
                        <polyline points="5,8 10,13 15,8"/>
                    </svg>
                </button>
                <div class="pensee-think-content">${finalContent.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')}</div>
            `;
            block.querySelector('.pensee-think-toggle').addEventListener('click', function() {
                const isOpen = block.classList.toggle('open');
                this.setAttribute('aria-expanded', String(isOpen));
            });
            _thinkWrapper.replaceWith(block);
            _thinkWrapper = null;
        }
        ...
        try {
            while (true) {
                ...
                // ── Détection token émotion \x02EM:{...}\x03 ───────────
                if (!_emotionHandled && chunk.includes('\x02EM:')) {
                    const emMatch = chunk.match(/\x02EM:(\{[^}]+\})\x03/);
                    if (emMatch) {
                        _emotionHandled = true;
                        // Appel du hook thinking-ui.js si disponible
                        if (window._penseeThinkingUI?.onEmotion) {
                            window._penseeThinkingUI.onEmotion(emMatch[1], bubble);
                        }
                        // On retire le token du chunk avant de l'ajouter à fullReply
                        const cleaned = chunk.replace(/\x02EM:[^]*?\x03/, '');
                        if (!cleaned.trim()) continue;
                        fullReply += cleaned;
                    } else {
                        // Token partiel — on accumule dans fullReply pour la prochaine itération
                        fullReply += chunk;
                    }
                    if (!document.hidden) {
                        bubble.innerHTML = formatResponse(_stripThinkTags(fullReply));
                        messagesEl.scrollTop = messagesEl.scrollHeight;
                    }
                    continue;
                }

                // ── Gestion des blocs <think>...</think> ───────────────
                if (hasThinking) {
                    let processed = chunk;

                    // Ouverture d'un bloc think
                    if (!_inThinkBlock && processed.includes('<think>')) {
                        _inThinkBlock = true;
                        _openThinkPanel();
                        // Sépare ce qui est avant <think> (réponse) et ce qui suit (thinking)
                        const [before, after] = processed.split(/<think>/i);
                        if (before.trim()) {
                            fullReply += before;
                            if (!document.hidden) {
                                bubble.innerHTML = formatResponse(fullReply);
                        ...
                        if (after !== undefined) {
                            _thinkContent += after;
                            if (_thinkLiveEl) {
                                _thinkLiveEl.textContent = _thinkContent;
                                _thinkLiveEl.scrollTop = _thinkLiveEl.scrollHeight;
                            }
                        }
                        continue;
                    }
                    ...
                    if (_inThinkBlock) {
                        if (processed.includes('</think>')) {
                            _inThinkBlock = false;
                            const [thinkEnd, afterThink] = processed.split(/<\/think>/i);
                            _thinkContent += thinkEnd;
                            _closeThinkPanel(_thinkContent.trim());
                            if (afterThink && afterThink.replace(/^\n/, '').trim()) {
                                fullReply += afterThink.replace(/^\n/, '');
                                if (!document.hidden) {
                                    bubble.innerHTML = formatResponse(fullReply);
                                    messagesEl.scrollTop = messagesEl.scrollHeight;
                                }
                            ...
                            _thinkContent += processed;
                            if (_thinkLiveEl && _showThinkLive()) {
                                _thinkLiveEl.textContent = _thinkContent;
                                _thinkLiveEl.scrollTop = _thinkLiveEl.scrollHeight;
                            }
                        }
            ...
            _streamDone = true;
            // Fermeture propre si le think n'a jamais été fermé (réponse tronquée)
            if (_inThinkBlock && _thinkWrapper) {
                _closeThinkPanel(_thinkContent.trim());
            }
        } catch (streamErr) {
        ...
        document.removeEventListener("visibilitychange", _handleVisibilityChange);
        // Fermeture propre du think si stream interrompu
        if (_inThinkBlock && _thinkWrapper) {
            clearInterval(_thinkTimerInt);
            _closeThinkPanel(_thinkContent.trim());
        }
        ...
        const pdfMarkerRegex = /\[GENERATE_PDF:\s*([^|]+)\|([\s\S]+)\][ \t]*$/i;
...
(async () => {
        ...
        // ── [K3] AUTO-SAVE KNOWLEDGE — non bloquant ──────────────
        // Évalue et sauvegarde silencieusement la réponse comme
        // exemple few-shot si elle dépasse le seuil de qualité.
        // Met à jour le profil utilisateur en parallèle.
        ...
        (async () => {
            try {
                ...
                if (!kToken || !fullReply || fullReply.length < 80) return;
                ...
                await fetch('/api/knowledge', {
                    ...
                    body: JSON.stringify({
                        action:     'save',
                        prompt:     userMessage,
                        response:   fullReply.slice(0, 2000),
                        agentId:    resolvedAgentId || 'default',
                        complexity: estimatePromptComplexity(userMessage),
                        domain:     resolvedAgentId || 'default',
                    }),
                });
...
async function sendMessage() {
    ...
    // Sauvegarde en DB + searchMemory en PARALLÈLE (gain ~350ms)
    const [, memoryResult] = await Promise.allSettled([
        saveMessageToDB("user", dbMessageText),
        searchMemory(messageText).catch(() => [])
    ]);

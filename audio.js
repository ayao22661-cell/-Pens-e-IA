...
function buildLiveOverlay() {
    // Bouton dans le header — créé statiquement dans le HTML, on attache juste l'event
    const existingBtn = document.getElementById('liveAudioBtn');
    if (existingBtn) {
        existingBtn.addEventListener('click', toggleLiveOverlay);
    } else {
        // Fallback : injection dynamique si le bouton HTML est absent
        ...
        if (headerRight) {
            ...
            btn.style.cssText = 'font-size:12px;background:none;border:1px solid var(--border2);color:var(--text2);border-radius:8px;padding:5px 10px;cursor:pointer;display:flex;align-items:center;gap:6px;';
